...
window.onload =function() {
    ...
    //时间轴
    function orderbytimestart() {
        var basePath = $("#txtRootPath").val();
        var content="";
        $.ajax({
            url:basePath + "/MyBlog/getArticleListOrderByTime"
            ,method:"post"
            ,dataType: "JSON"
            ...
            ,success: function(data) {
                ...
                if(data != null){
                    ...
                    $.each(temp,function(index,temp){
                        content += "<li><span>"+ temp['release_time'] +"</span><a href='info.jsp?article_id=" + temp['article_id'] + "' title='"+ temp['author'] +"'>" + temp['article_title'] + "</a></li>";
                    });
                    //动态将li 写入ul
                    document.getElementById("list2").innerHTML=content;
                }
            }
        });
    }
